namespace _10._ForceBook
{
    ...
    public class StartUp
    {
        static void Main()
        {
            ...
            while ((inputLineFromConsole = Console.ReadLine()) != "Lumpawaroo")
            {
                if (inputLineFromConsole.Contains(" | "))
                    AddUserToSide(inputLineFromConsole, userSideDict);
                else if (inputLineFromConsole.Contains(" -> "))
                    ChangeUserSide(inputLineFromConsole, userSideDict);
            }
            IO(userSideDict);
        }
        ...
        static void IO(Dictionary<string, List<string>> userSideDict)
        {
